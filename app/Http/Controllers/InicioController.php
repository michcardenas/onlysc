    ...
    public function show($nombreCiudad, $sector = null, $filtros = null)
    {
        ...
        try {
            ...
            $usuarios = $query->with([
                ...
                'posts' // Incluir la relación posts
            ])
                ->select(
                    'id',
                    'fantasia',
            ...
            // Add $isPrecio variable initialization
            $isPrecio = false;
            $isEdad = false;
            $isCategorias = false;
            $isZona = false;
            ...
            if (count($pathParts) > 1) {
                ...
                // Check if zona exists in table
                $isZona = DB::table('escort_locations')
                ...
                $isNacionalidad = in_array($pathParts[1], [
                    'colombiana',
                    'argentina',
                    'brasilena',
                    'chilena',
                ...
                $isAtributo = in_array($pathParts[1], [
                    "busto-grande",
                    "busto-mediano",
                    "busto-pequeño",
                    "cara-visible",
                ...
                $isServicio = $isNacionalidad = $isAtributo = false;
            }

            // Check for resena-verificada
$isResenaVerificada = ($pathParts[1] ?? '') === 'resena-verificada';


            // Get meta tag based on the type of filter
            $metaTag = MetaTag::firstWhere('page', $routeKey);
            if (!$metaTag) {
                if ($isResenaVerificada) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/escorts-con-resenas');
                } elseif ($isServicio) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/servicios');
                } elseif ($isNacionalidad) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/escorts-nacionalidad');
                } elseif ($isAtributo) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/atributos');
                } elseif ($isPrecio) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/precio');
                } elseif ($isEdad) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/edad');
                } elseif ($isCategorias) {
                    $metaTag = MetaTag::firstWhere('page', 'seo/escorts-categoria');
                } elseif ($isZona) {
                    $metaTag = MetaTag::firstWhere('page', 'seo');
                } else {
                    // Si no hay filtros o no coincide con ninguno, usar el meta tag original
                    $metaTag = MetaTag::where('page', 'inicio-' . $ciudadSeleccionada->id)->first();
                }
            }

            // Share the meta information with the view
            view()->share([
                'pageTitle' => $metaTag?->meta_title ?? $title,
